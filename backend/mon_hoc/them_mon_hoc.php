...
try {
    ...
    if ($khoa["trang_thai"] !== "dang_hoat_dong") {
        ...
        echo json_encode([
            "status" => "error",
            "message" => "Không thể thêm môn học vào khoa đã ngừng hoạt động"
        ], JSON_UNESCAPED_UNICODE);
        exit();
    }
    ...
    if ($boMon["trang_thai"] !== "dang_hoat_dong") {
        ...
        echo json_encode([
            "status" => "error",
            "message" => "Không thể thêm môn học vào bộ môn đã ngừng hoạt động"
        ], JSON_UNESCAPED_UNICODE);
        exit();
    }
    ...
    $checkTenSql = "SELECT id 
                    FROM mon_hoc 
                    WHERE ten_mon = :ten_mon AND bo_mon_id = :bo_mon_id 
                    LIMIT 1";
