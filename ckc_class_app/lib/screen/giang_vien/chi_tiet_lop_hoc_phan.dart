...
import 'quan_ly_bai_tap.dart';
import 'quan_ly_thong_bao.dart';
import 'danh_sach_sinh_vien_lop.dart';

class ChiTietLopHocPhan extends StatefulWidget {
  final LopHocPhan lop;
  const ChiTietLopHocPhan({super.key, required this.lop});

  @override
  State<ChiTietLopHocPhan> createState() => _ChiTietLopHocPhanState();
}

class _ChiTietLopHocPhanState extends State<ChiTietLopHocPhan>
  ...
  Widget _buildThongTinLop(LopHocPhan lop) {
    return Container(
      ...
      child: Stack(
        children: [
          Positioned(right: -28, top: -34, child: _DecorCircle(size: 110, opacity: 0.14)),
          Positioned(right: 40, bottom: -48, child: _DecorCircle(size: 92, opacity: 0.10)),
          Column(
            ...
            children: [
              Row(
                ...
                children: [
                  Container(
                    ...
                    child: const Icon(Icons.class_rounded, color: Colors.white, size: 30),
                  ),
              ...
              Wrap(
                ...
                runSpacing: 8,
                children: [
                  _InfoPill(icon: Icons.badge_rounded, text: lop.maLopHocPhan),
                  if (lop.hocKy != null)
                    _InfoPill(
                      icon: Icons.calendar_month_rounded,
                      text: '${lop.hocKy}${lop.namHoc != null ? ' • ${lop.namHoc}' : ''}',
                    ),
                  if (lop.tinChi != null)
                    _InfoPill(icon: Icons.school_rounded, text: '${lop.tinChi} tín chỉ'),
                  _InfoPill(icon: Icons.groups_rounded, text: '${lop.soSinhVien} sinh viên'),
                  _InfoPill(icon: Icons.assignment_rounded, text: '${lop.soBaiTap} bài tập'),
                  _InfoPill(
                    icon: lop.isDangMo ? Icons.lock_open_rounded : Icons.lock_rounded,
                    text: lop.tenTrangThai,
                  ),
                ],
              ),
            ],
